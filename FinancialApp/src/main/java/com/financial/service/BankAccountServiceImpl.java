...
public class BankAccountServiceImpl implements BankAccountService {
	...
	public BankAccountDto addBankDetails(BankAccountDto bankAccountDto, Integer userid)
		...
		boolean bankAccountExists = bankAccountRepo.existsByUsersAndAccountNumber(users,
				bankAccountDto.getAccountNumber());
		if (bankAccountExists) {
			throw new BankAccountNotFoundException("Bank account already exists with account number '"
					+ bankAccountDto.getAccountNumber() + "' for user with ID: " + userid);
		}
	...
	public String removeBankAccountDetails(Integer userid, Integer bankId)
		...
		BankAccounts bankAccounts = bankAccountRepo.findById(bankId).orElseThrow(
				() -> new BankAccountNotFoundException("Bank Account id not found with this Id :" + bankId));

		if (!bankAccounts.getUsers().equals(users)) {
			throw new BankAccountNotFoundException(
					"Bank account not found with ID: " + bankId + " for user with ID: " + userid);
		}
